    ...
    <div class="flex item-center grid grid-cols-3 gap-3 mt-4 m-2 p-2">
        ...
        <div>
            ...
            <div x-data="imageSlider"
                class="relative mx-auto max-w-2xl overflow-hidden rounded-md bg-gray-100 p-2 sm:p-4 mt-4">
                ...
                <div class="flex item-center grid grid-cols-2 gap-2 mt-4">
                    <div>
                        <div class="flex -space-x-2 overflow-hidden">
                            <img class="inline-block h-20 w-20 rounded-full ring-4 ring-yellow-600 m-2"
                                src="{{$latestsender->user->avatar?$latestsender->user->avatar:'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80'}}"
                                alt="">
                        </div>
                    </div>
                    <div>
                        ...
                        <h4>Location: {{$latestsender->fire->address?$latestsender->fire->address : 'No address
                            available'}}</h4>
                    </div>
                </div>
                <form method="post" action="{{ route('post.update') }}" autocomplete="off">
                    ...
                    <div class="flex item-center grid grid-cols-2 gap-2 mt-4">
                        <div class="flex min-w-0 gap-x-4">
                            <div class="min-w-0 flex-auto">
                                ...
                                <select name="fire_type" id="fire_type" class="rounded max-w-full">
                                    ...
                                    <option value="{{ $firetype }}" {{ $latestsender->fire_type == $firetype ?
                                        'selected' : '' }}>
                                        {{ $firetype }}
                                    </option>
                                    @endforeach
                        ...
                        <div class="place-self-center">
                            <button type="submit"
                                class="p-2 pl-6 pr-6 bg-red-500 rounded-full text-white text-sm shadow-lg hover:shadow-red-500/50 hover:duration-700">SEND
                                ALARM</button>
                        </div>
                    </div>
                </form>
                    ...
                    <form method="post" action="{{ route('notify.stations', $station->id) }}" autocomplete="off">
                        ...
                        <li class="flex justify-between gap-x-6 py-5 items-center">
                            ...
                            <div class="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
                                <button
                                    class="pl-6 pr-6 bg-red-600 rounded-full text-white text-sm p-1 shadow-lg hover:shadow-red-500/50 hover:duration-700">SEND</button>
                            </div>
                        </li>
                    </form>
                    @endforeach
                ...
                <ul role="list" class="divide-y divide-gray-100 overflow-scroll max-h-screen">
                    ...
                    <li
                        ...
                        <a href="{{route('dashboard.show', $latest->id)}}">
                            <div class="flex min-w-0 gap-x-4">
                                <img class="h-12 w-12 flex-none rounded-full bg-gray-50 ring-4 ring-yellow-600 m-2"
                                    src="{{$latest->user->avatar?$latest->user->avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80'}}"
                                    alt="">
                                <div class="min-w-0 flex-auto">
            ...
            <div class="relative w-full overflow-hidden rounded-md bg-gray-100 p-2 sm:p-4 mt-4">
                <div class="flex min-w-0 gap-x-4">
                    ...
                    <div class="min-w-0 flex-auto">
                        ...
                        <p class="mt-1 truncate text-xs leading-5 text-gray-500">Age: {{ $latest->user->age?
                            $latest->user->age: 0 }}</p>
                        <p class="mt-1 truncate text-xs leading-5 text-gray-500">Location: {{$latest->fire->address ?
                            $latest->fire->address : 'No address available'}}</p>
                    </div>
                </div>
                <div class="flex min-w-0 gap-x-4">
                    ...
                    <div class="min-w-0 flex-auto">
                        ...
                        <p class="mt-1 truncate text-xs leading-5 text-gray-500">Type of fire: {{ $latest->fire_type }}
                        </p>
